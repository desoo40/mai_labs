...
sData *list_node_data_create(int key, int value); // создает и инициализирует структуру данных узла
...
int list_size(List *list); // возвращает размер
int list_lenght(List *list); // возвращает длину
...
ListNode *find_elem(List *list, int key); // ищет элемент по ключу
